<?php
/**
 * Sidebar para modulo de Inventario de EPP
 * includes/sidebar/sidebar_inventario.php
 */
...
try {
    $pdo = conectarDB();
    $stmt_ss = $pdo->query("SELECT COUNT(*) as total FROM inventario_epp WHERE activo = 1 AND stock = 0");
    $epp_sin_stock = $stmt_ss->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
} catch (Exception $e) { $epp_sin_stock = 0; }
...
?>
<button class="hamburger-btn" type="button" aria-label="Abrir menu" aria-expanded="false" aria-controls="sidebar"><span class="hamburger-icon"><span></span><span></span><span></span></span></button>
<div class="sidebar-overlay" aria-hidden="true"></div>
<aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <i class="bi bi-shield-check text-white fs-1 mb-2"></i>
        <h4><?php echo htmlspecialchars($_SESSION['departamento_nombre'] ?? 'EPP'); ?></h4>
        <small class="text-white-50"><?php echo htmlspecialchars($_SESSION['nombre_completo']); ?></small>
        <span class="badge bg-warning text-dark mt-2">Inventario EPP</span>
    </div>
    <nav class="sidebar-nav">
        <ul class="nav flex-column">
            <li class="nav-item"><a class="nav-link <?php echo $current_page == 'dashboard_epp.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/inventario_epp/dashboard_epp.php"><i class="bi bi-house-door"></i> Inicio</a></li>
            ...
            <li class="nav-item"><a class="nav-link <?php echo $en_inventario ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/inventario_epp/inventario_epp.php"><i class="bi bi-box-seam"></i> Ver Inventario</a></li>
            <?php if ($permisos_epp['puede_crear']): ?>
            <li class="nav-item"><a class="nav-link <?php echo $en_agregar ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/inventario_epp/agregar_epp.php"><i class="bi bi-plus-circle"></i> Agregar EPP</a></li>
            <li class="nav-item"><a class="nav-link <?php echo $en_registrar_mov ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/inventario_epp/registrar_movimiento.php"><i class="bi bi-pencil-square"></i> Registrar Movimiento</a></li>
            <?php endif; ?>
            <?php if ($epp_sin_stock > 0): ?>
            <li class="nav-item"><a class="nav-link text-warning" href="<?php echo URL_BASE; ?>dashboard/inventario_epp/inventario_epp.php?vista=inventario"><i class="bi bi-exclamation-triangle"></i> Sin Stock <span class="badge bg-danger ms-2"><?php echo $epp_sin_stock; ?></span></a></li>
            <?php endif; ?>
            <hr class="text-white-50 my-2">
