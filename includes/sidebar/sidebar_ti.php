...
$en_gestion_usuarios = in_array($current_page, ['dashboard_usuarios.php','crear_usuario.php','editar_usuario.php']);
try {
    if (!isset($pdo)) { require_once __DIR__ . '/../../config/database.php'; $pdo = conectarDB(); }
    $stmt = $pdo->query("SELECT SUM(CASE WHEN estado = 'pendiente' THEN 1 ELSE 0 END) as pendientes, SUM(CASE WHEN estado = 'en_proceso' THEN 1 ELSE 0 END) as en_proceso FROM solicitudes_atencion");
    $stats_sidebar = $stmt->fetch();
    $stmt_ordenes = $pdo->prepare("SELECT COUNT(*) as total FROM ordenes_servicio_mantenimiento WHERE usuario_id = :uid AND estado = 'pendiente_usuario'");
    $stmt_ordenes->execute([':uid' => $_SESSION['usuario_id']]);
    ...
    try { $stmt_eq = $pdo->query("SELECT COUNT(*) as total FROM inventario_equipos WHERE estado != 'dado_de_baja'"); $total_equipos = $stmt_eq->fetch(PDO::FETCH_ASSOC)['total'] ?? 0; } catch(Exception $e) { $total_equipos = 0; }
    try { $stmt_mp = $pdo->query("SELECT COUNT(*) as total FROM solicitudes_mantenimiento_ti WHERE estado = 'pendiente'"); $mant_pendientes = $stmt_mp->fetch(PDO::FETCH_ASSOC)['total'] ?? 0; } catch(Exception $e) { $mant_pendientes = 0; }
} catch (Exception $e) {
    $stats_sidebar = ['pendientes' => 0, 'en_proceso' => 0]; $ordenes_pendientes_validar = 0; $total_usuarios = 0; $total_equipos = 0; $mant_pendientes = 0;
}
?>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        <ul class="nav flex-column">
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'ti_sistemas.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/sistemas/ti_sistemas.php">
                    <i class="bi bi-house-door"></i> Inicio
                </a>
            </li>
            
            <hr class="text-white-50 my-2">
            <small class="text-white-50 px-3 fw-bold">GESTION DE SOLICITUDES</small>
            
            <li class="nav-item">
                <a class="nav-link <?php echo ($current_page == 'gestion_solicitudes.php' && empty($filtro_estado_actual)) ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>ti_sistemas/gestion_solicitudes.php">
            ...
            <hr class="text-white-50 my-2">
            <small class="text-white-50 px-3 fw-bold">ADMINISTRACION DE USUARIOS</small>
            
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'dashboard_usuarios.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/sistemas/gestion_usuarios/dashboard_usuarios.php">
            ...
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'crear_usuario.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/sistemas/gestion_usuarios/crear_usuario.php">
            ...
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'reservaciones_sala.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/reservaciones/reservaciones_sala.php">
            ...
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'mis_vacaciones.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/vacaciones/mis_vacaciones.php">
                    <i class="bi bi-calendar-check"></i> Mis Vacaciones
                </a>
            </li>
            <?php if (function_exists('es_admin_area') && es_admin_area()): ?>
            <li class="nav-item">
                <a class="nav-link <?php echo $current_page == 'vacaciones_admin.php' ? 'active' : ''; ?>" href="<?php echo URL_BASE; ?>dashboard/vacaciones/vacaciones_admin.php">
