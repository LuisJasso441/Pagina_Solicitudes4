<?php
/**
 * Handler: editar comentario propio
 * dashboard/salidas_envases/editar_comentario_sec.php
 */
...
$resultado = editar_comentario_sec($comentario_id, $usuario_id, $texto_nuevo);

if ($resultado['success']) {
    if (empty($resultado['sin_cambios'])) {
        // Solo registrar historial si hubo cambios reales
        registrar_historial_sec(
            $resultado['sec_id'], $usuario_id, 'comentario_editado',
            'Edito un comentario',
            [
                'comentario_id' => $comentario_id,
                'texto_original' => $resultado['texto_original'] ?? '',
                'texto_nuevo'    => $resultado['texto_nuevo'] ?? ''
            ]
        );
    }
    redirigir(URL_BASE . "dashboard/salidas_envases/ver_sec.php?id=$sec_id&msg=comentario_editado#comentarios");
} else {
