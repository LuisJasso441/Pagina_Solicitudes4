...
/**
 * Nueva Reservación
 * Ubicación: dashboard/reservaciones/nueva_reservacion.php
 */
session_start();
...
$depto_id_usuario = $_SESSION['departamento_id'] ?? null;

$form_data = $_SESSION['form_data'] ?? [];
$errores = $_SESSION['form_errors'] ?? [];
unset($_SESSION['form_data'], $_SESSION['form_errors']);

$fecha_pre = $_GET['fecha'] ?? ($form_data['fecha_reservacion'] ?? '');
$hora_pre = $_GET['hora'] ?? '';
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Nueva Reservación - <?php echo NOMBRE_SISTEMA; ?></title>
...
<body>
        ...
        <main class="main-content">
            <div class="container-fluid p-3">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h4 class="mb-0"><i class="bi bi-plus-circle me-2"></i>Nueva Reservación</h4>
                        <small class="text-muted">Sala de Juntas / Capacitaciones</small>
                    </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control" id="solicitante_nombre" name="solicitante_nombre"
                                           value="<?php echo htmlspecialchars($form_data['solicitante_nombre'] ?? $nombre_usuario); ?>" required>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" id="departamento_id" name="departamento_id" required>
                                        ...
                                        <option value="<?php echo $d['id']; ?>" <?php echo (($form_data['departamento_id'] ?? $depto_id_usuario) == $d['id']) ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" id="lugar" name="lugar" required>
                                        ...
                                        <option value="<?php echo $cod; ?>" <?php echo (($form_data['lugar'] ?? 'sala_juntas') === $cod) ? 'selected' : ''; ?>><?php echo $nom; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label for="fecha_reservacion" class="form-label fw-semibold">Fecha de la Reservación <span class="text-danger">*</span></label>
                                    <input type="date" class="form-control" id="fecha_reservacion" name="fecha_reservacion"
                                           value="<?php echo htmlspecialchars($fecha_pre); ?>" min="<?php echo date('Y-m-d'); ?>" required>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="hora_inicio" name="hora_inicio" placeholder="HH:MM" maxlength="5" 
                                               value="<?php echo htmlspecialchars($form_data['hora_inicio'] ?? ''); ?>" required>
                                        <select class="form-select" id="ampm_inicio" name="ampm_inicio" style="max-width:75px">
                                            <option value="AM" <?php echo ($form_data['ampm_inicio'] ?? 'AM') === 'AM' ? 'selected' : ''; ?>>AM</option>
                                            <option value="PM" <?php echo ($form_data['ampm_inicio'] ?? '') === 'PM' ? 'selected' : ''; ?>>PM</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="hora_fin" name="hora_fin" placeholder="HH:MM" maxlength="5"
                                               value="<?php echo htmlspecialchars($form_data['hora_fin'] ?? ''); ?>" required>
                                        <select class="form-select" id="ampm_fin" name="ampm_fin" style="max-width:75px">
                                            <option value="AM" <?php echo ($form_data['ampm_fin'] ?? 'AM') === 'AM' ? 'selected' : ''; ?>>AM</option>
                                            <option value="PM" <?php echo ($form_data['ampm_fin'] ?? '') === 'PM' ? 'selected' : ''; ?>>PM</option>
                                        </select>
                                    </div>
                                </div>
                                <!-- MOTIVO -->
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control" id="motivo" name="motivo" maxlength="255"
                                           ...
                                           value="<?php echo htmlspecialchars($form_data['motivo'] ?? ''); ?>">
                                </div>
                            </div>
                            <hr class="my-3">
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-success" id="btn-reservar"><i class="bi bi-check-circle me-1"></i> Reservar</button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const horaPre = '<?php echo addslashes($hora_pre); ?>';
        if (horaPre) {
            const [h, m] = horaPre.split(':');
            let hora = parseInt(h); const ampm = hora >= 12 ? 'PM' : 'AM';
            if (hora > 12) hora -= 12; if (hora === 0) hora = 12;
            document.getElementById('hora_inicio').value = String(hora).padStart(2,'0') + ':' + m;
            document.getElementById('ampm_inicio').value = ampm;
        }
        ...
        function verificarDisponibilidad() {
            ...
            fetch(`${URL_BASE}api/reservaciones/verificar_disponibilidad.php`, {
                ...
                body: JSON.stringify({ fecha, hora_inicio: inicio24, hora_fin: fin24, lugar, excluir_id: 0 })
                ...
                } else if (data.error) { conflictoTexto.textContent = data.error; alertaConflicto.classList.remove('d-none'); hayConflicto = true; btnReservar.disabled = true; }
