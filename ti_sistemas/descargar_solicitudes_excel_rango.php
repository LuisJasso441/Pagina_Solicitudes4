<?php
/**
 * descargar_solicitudes_excel_rango.php
 * Genera y descarga un archivo Excel con los datos de Solicitudes de Atención
 * FILTRADO POR RANGO DE FECHAS
 * 
 * Ubicación: ti_sistemas/descargar_solicitudes_excel_rango.php
 */
...
if (empty($fechaDesde) || empty($fechaHasta)) {
    $_SESSION['error'] = "Debe especificar ambas fechas para el rango.";
    header('Location: gestion_solicitudes.php');
    exit;
}
...
if (!file_exists($autoloadPath)) {
    $_SESSION['error'] = "PhpSpreadsheet no está instalado.";
    header('Location: gestion_solicitudes.php');
    exit;
}
...
try {
    ...
    // Obtener solicitudes en el rango de fechas
    $sql = "SELECT 
                s.id,
                s.folio,
                s.departamento,
    ...
    if (empty($solicitudes)) {
        $_SESSION['error'] = "No hay solicitudes en el rango de fechas seleccionado.";
        header('Location: gestion_solicitudes.php');
        exit;
    }
    ...
    $rangoTexto = $fechaDesdeFormato . ' al ' . $fechaHastaFormato;
    ...
    // Estilo de encabezados - Color CELESTE para diferenciar del mes actual
    $headerStyle = [
        'font' => [
            'bold' => true,
            'color' => ['rgb' => 'FFFFFF'],
        ...
        'fill' => [
            ...
            'startColor' => ['rgb' => '17A2B8'] // Celeste/Info para rango de fechas
        ],
        'alignment' => [
    ...
    // Estilo para títulos de tablas - Color CELESTE
    $titleStyle = [
        'font' => [
            'bold' => true,
            'size' => 12,
        ...
        'fill' => [
            'fillType' => Fill::FILL_SOLID,
            'startColor' => ['rgb' => '17A2B8'] // Celeste para rango
        ],
        'alignment' => [
            'horizontal' => Alignment::HORIZONTAL_CENTER,
            'vertical' => Alignment::VERTICAL_CENTER
    ...
    $filename = 'Solicitudes_Atencion_' . str_replace('/', '-', $fechaDesdeFormato) . '_a_' . str_replace('/', '-', $fechaHastaFormato) . '.xlsx';
    ...
    // Log de descarga
    error_log("Excel por rango descargado - Rango: {$rangoTexto} - Solicitudes: " . count($solicitudes));
    
    exit;
    
} catch (Exception $e) {
    error_log("Error al generar Excel de solicitudes por rango: " . $e->getMessage());
